...
{
    /// 環控區域劃分管理器
    public class EnvironmentSectionManager : MonoBehaviour
    {
        #region Variables
        ...
        [Label("[資料項 - EnvironmentSection]"), SerializeField]
        private List<EnvironmentSection> environmentSections;
        ...
        [Foldout("[Event] - 所有Section都未選取時Invoke")]
        public UnityEvent onNonSelectedSectionEvent;
        ...
        [Button]
        private void GetEnvDataHoldersInAreaBySections() =>
            environmentSections.ForEach(section => section.GetEnvDataHoldersInArea());
        ...
        private void CreateEnvValueDisplaysBySettings()
        {
            environmentSections.ForEach(section =>
            {
                ...
                {
                    EnvironmentValueDisplay envValueDisplay =
                        ObjectHelper.Instantiate(setting.envDisplayPrefab, setting.container);
        ...
        [Button]
        private void DeleteAllEnvValueDisplays() =>
            environmentSections.ForEach(section => section.ClearEnvDisplayTarget());
        ...
        public void CalculateAverageValue()
        {
            ...
            environmentSections.ForEach(section =>section.CalculateAverageEnvData());
            
            float allAverageRt = 0, allAverageRh = 0;

            List<EnvironmentSection> filter;
            filter = environmentSections.Where(section => section.AverageRt >= 0).ToList();
            allAverageRt = filter.Count>0? filter.Average(section => section.AverageRt) : -1;
            
            filter = environmentSections.Where(section => section.AverageRh >= 0).ToList();
            allAverageRh = filter.Count>0? filter.Average(section => section.AverageRh) : -1;
        ...
        {
            if (environmentSections.Count == 0)
                environmentSections = FindObjectsByType<EnvironmentSection>(FindObjectsSortMode.None).ToList();
