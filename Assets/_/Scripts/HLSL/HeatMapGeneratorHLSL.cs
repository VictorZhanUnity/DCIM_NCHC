...
namespace VictorDev.HeatMapUtiils
{
    ...
    {
        [Button]
        public void SetHeapMapPoint() => SetHeapMapPoint(heatMapSetting.TestHeatMapPoint);
        ...
        public void Generate()
        {
            ...
            {
                for (int xLoop = 0; xLoop <= countX; xLoop++)
                {
                    float x = xLoop * spacing;
                    for (int yLoop = 0; yLoop <= countY; yLoop++)
                    {
                        float y = yLoop * spacing;
                        for (int zLoop = 0; zLoop <= countZ; zLoop++)
                        {
                            float z = zLoop * spacing;
                            ...
                            if (BoxColliderTarget.bounds.Contains(worldPos))
                            {
                                ...
                                float emission = Mathf.Lerp(0f, emissionIntensity, value);
